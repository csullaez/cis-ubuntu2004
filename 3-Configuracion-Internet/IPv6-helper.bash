...
function check_ip_v6(){
  ...
  [ -f "$grubfile" ] && ! grep "^\s*linux" "$grubfile" | grep -vq ipv6.disable=1 && output="ipv6 desactivado en grub config"
  ...
  grep -Eq "^\s*net\.ipv6\.conf\.default\.disable_ipv6\s*=\s*1\b" && output="ipv6 desactivado en sysctl config"

  [ -n "$output" ] && echo -e "\n$output" || echo -e "\n*** IPv6 desactivado en el sistema ***"
}
